...
class Event:
    ...
    def _parse_shares_and_fees(cls, event_dict: Dict[Any, Any]) -> Tuple[Optional[float], Optional[float]]:
        ...
        sections = event_dict.get("details", {}).get("sections", [{}])
        transaction_dicts = list(filter(lambda x: x.get("title") in ["Transaktion", "Transaction"], sections))
        for transaction_dict in transaction_dicts:
            dump_dict["maintitle"] = transaction_dict["title"]
            data = transaction_dict.get("data", [{}])
            shares_dicts = list(filter(lambda x: x["title"] in ["Aktien", "Anteile", "Shares", "Debited Shares"], data))
        ...
        if shares is None:
            overview_dicts = list(filter(lambda x: x.get("title") in ["Overview"], sections))
            if not overview_dicts:
                return None, None
            overview_dict = overview_dicts[0]
            data = overview_dict.get("data", [{}])
            transaction_dicts = list(filter(lambda x: x.get("title") in ["Transaction"], data))
            if not transaction_dicts:
                return None, None
            transaction_dict = transaction_dicts[0]
            transaction_prefix = transaction_dict.get("detail", {}).get("displayValue", {}).get("prefix", "")
            shares_str = re.split("\s", transaction_prefix, 1)[0]
            shares = float(parse_decimal(shares_str, "en", strict=True))
